...
// milos, added - effectId/effectBlockIndex always comes straight from a host-controlled USB byte
// (legal range up to 40 per our own HID report descriptor, or 0xFF/0x7F as an "all effects" sentinel),
// while gEffectStates[]/effectTime[]/gDisabledEffects.effectId[] are only MAX_EFFECTS(+1) elements long.
// Every access keyed by a raw host value must go through this check first.
bool IsValidEffectId(uint8_t id) { return id >= FIRST_EID && id <= MAX_EFFECTS; } // dustin's rig, changed from macro to a real function - saves flash by sharing one copy across ~11 call sites instead of duplicating the comparison inline at each one
...
void FfbHandle_EffectOperation(USB_FFBReport_EffectOperation_Output_Data_t *data)
{
  ...
  { // StartSolo
    ...
    StopAllEffects(); // milos, this already calls the driver's StopEffect() for every effect that was actually playing, safely
...
void FfbHandle_DeviceGain(USB_FFBReport_DeviceGain_Output_Data_t *data)
{
  ...
  */
  gDeviceGain = data->deviceGain; // milos, fixed - this used to be a no-op, so a game/host lowering its Device Gain had no effect on actual FFB strength; now applied in ffb_pro.ino's final torque scaling
