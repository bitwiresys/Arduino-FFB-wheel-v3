...
//#define USE_QUADRATURE_ENCODER		// milos, optical quadrature encoder (you may comment it out) // dustin's rig: disabled, using AS5600 magnetic encoder instead
...
#define USE_AS5600          // milos, uncomment to enable magnetic encoder via i2C instead of optical encoder // dustin's rig: Leonardo + AS5600 + BTS7960
// dustin's rig, added - AS5600 already applies its own onboard 16x "slow filter" (see setSlowFilter(0) call), but
// residual ~1deg jitter is common with these cheap breakout boards (magnet air-gap/centering tolerance, i2C/ADC noise).
// AS5600_HYSTERESIS suppresses single-LSB toggling while stationary (0=off,1=1LSB,2=2LSB,3=3LSB - see AS5600 datasheet).
// AS5600_EMA_ALPHA_Q8 is an extra software low-pass on top of that, in Q8 fixed point (0=frozen/max smoothing, 256=raw/no smoothing).
// Lower it further (e.g. 48-64) for more smoothing at the cost of a bit more FFB latency, raise it if the wheel feels sluggish.
#define AS5600_HYSTERESIS    1
#define AS5600_EMA_ALPHA_Q8  90
...
#define RCM_MIN_BASE 1.0 // milos, added - canonical (unscaled) minimal RCM pulse width in ms, used by ReconfigurePWMMode() so repeated calls (e.g. runtime 'W' command) don't compound the RCMscaler() multiplication
#define RCM_ZER_BASE 1.5 // milos, added - canonical (unscaled) zero RCM pulse width in ms
#define RCM_MAX_BASE 2.0 // milos, added - canonical (unscaled) maximal RCM pulse width in ms
f32 RCM_min = RCM_MIN_BASE; // minimal RCM pulse width in ms
f32 RCM_zer = RCM_ZER_BASE; // zero RCM pulse width in ms
f32 RCM_max = RCM_MAX_BASE; // maximal RCM pulse width in ms
