...
#ifdef USE_AS5600
s32 gAS5600FilteredX = 0; // dustin's rig, added - EMA filter state for AS5600 x-axis angle, on top of the sensor's own onboard slow filter, to further reduce ~1deg jitter
#ifdef USE_TWOFFBAXIS
s32 gAS5600FilteredY = 0; // dustin's rig, added - EMA filter state for AS5600 y-axis angle (2nd sensor)
...
void setup() {
  ...
  as5600x.setHysteresis(AS5600_HYSTERESIS); // dustin's rig, added - suppress single-LSB toggling noise while the wheel is stationary
  ...
  gAS5600FilteredX = 0; // dustin's rig, added - seed the EMA filter so it doesn't ramp up from 0 after the reset above
  ...
  as5600y.setHysteresis(AS5600_HYSTERESIS); // dustin's rig, added
  as5600y.resetCumulativePosition(ROTATION_MID);
  gAS5600FilteredY = 0; // dustin's rig, added
#endif // end of tca
#endif // end of 2 ffb axis
#endif // end of as5600
  last_refresh = micros();
  ...
  now_micros = micros(); // milos, we are polling the loop (FFB and USB reports are sent periodicaly)
  {
    ...
    if ((now_micros - last_refresh) >= CONTROL_PERIOD) {
      ...
      if (cButtonPressed) { // milos, reset magnetic encoders to 0 deg
        ...
        gAS5600FilteredX = turn.x - ROTATION_MID; // dustin's rig, added - re-seed the EMA filter so recentering doesn't slowly ramp in
#ifdef USE_TCA9548
        ...
        gAS5600FilteredY = turn.y - ROTATION_MID; // dustin's rig, added
#endif // end of tca
      ...
      TcaChannelSel(baseTCA0, 0); // milos, select 1st i2C channel for AS5600(0x36) on x-axis
#endif // end of tca
      gAS5600FilteredX += (((s32)as5600x.getCumulativePosition() - ROTATION_MID - gAS5600FilteredX) * AS5600_EMA_ALPHA_Q8) >> 8; // dustin's rig, added - extra software low-pass on top of the sensor's own filter
      turn.x = gAS5600FilteredX; // milos, AS5600 angle readout
#endif // end of as5600
      ...
      gAS5600FilteredY += (((s32)as5600y.getCumulativePosition() - ROTATION_MID - gAS5600FilteredY) * AS5600_EMA_ALPHA_Q8) >> 8; // dustin's rig, added
      turn.y = gAS5600FilteredY; // milos, 2nd AS5600 angle readout
      ...
      // USB Report
      {
        ...
        SendAxisReport(turn.x + MID_REPORT_X + 1, brake.val, accel.val, clutch.val, hbrake.val, button); // milos, X, Y, Z, RX, RY, hat+button; (0-65535) X-axis range, center at 32768
#else // milos, if no quad enc
#ifdef USE_AS5600 // milos, if we use one as5600
#ifndef USE_TCA9548 // milos, if we don't use two as5600
        SendAxisReport(turn.x + MID_REPORT_X + 1, brake.val, accel.val, clutch.val, hbrake.val, button); // milos, one as5600 at x-axis
#else // with tca, if two as5600
        SendAxisReport(turn.x + MID_REPORT_Y + 1, turn.y + MID_REPORT_Y + 1, accel.val, clutch.val, hbrake.val, button); // milos, we use two as5600, send 2nd as5600 at y-axis instead of brake pedal
#endif // end of tca
        ...
        SendAxisReport(turn.x + MID_REPORT_X + 1, brake.val, Z_AXIS_PHYS_MAX >> 1, clutch.val, hbrake.val, button); // milos
#else // milos, when usign split axis, we have one more uncalibrated axis available to use for Z-axis
        SendAxisReport(turn.x + MID_REPORT_X + 1, brake.val, gasAxis, clutch.val, hbrake.val, button); // milos, full analog joystick
